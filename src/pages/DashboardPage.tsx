...
import { calculateDashboardSummary } from '../features/dashboard/dashboardUtils'
...
function DashboardPage() {
  ...
  const normalSessions = useMemo(
    () => sessions.filter((session) => session.mode === 'normal'),
    [sessions],
  )
  const summary = useMemo(
    () => calculateDashboardSummary(normalSessions),
    [normalSessions],
  )
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === STUDY_SESSIONS_STORAGE_KEY) refreshSessions()
    }
  ...
  return (
    <section className="page page--wide">
      ...
      <h1>Dashboard</h1>
      <p>See where your normal study time is going and keep momentum visible.</p>

      {normalSessions.length === 0 ? (
        <div className="dashboard-empty-state">
          <h2>No normal study data yet</h2>
          <p>Complete a normal timer session to start building your dashboard.</p>
        </div>
      ) : (
